...
! =============================================================================
!
! METIS is an open-source software, which allows scientists to build and
...
subroutine Main()
    ...
    if(iargc() == 0) then
        call Verify_Solution(prob, mesh, dna)
...
subroutine Print_Summary(prob, geom, mesh, dna)
    ...
    write(p_redir, "(a)"), "   * Min. gap dsDNA domain for nick : "//trim(adjustl(Int2Str(para_gap_min_region)))
    write(p_redir, "(a)"), "   * Gap b/w xover and nick         : "//trim(adjustl(Int2Str(para_gap_xover_nick)))
    write(p_redir, "(a)"), "   * Staple break rule              : "//trim(para_break_stap_rule)
    ...
    write(p_redir, "(a)"), " | METIS generated output files.                     |"
    write(p_redir, "(a)"), " +===================================================+"
...
! Verify solution
subroutine Verify_Solution(prob, mesh, dna)
    type(ProbType), intent(in) :: prob
    type(MeshType), intent(in) :: mesh
    type(DNAType),  intent(in) :: dna
    ...
    write(0, "(a)"), "    2.42920867312334E+04 - Ref.: 1-1"
    write(0, "(a)"), "    7.83270490787087E+04 - Ref.: 2-1"
    write(0, "(a)"), "    1.64544992443105E+05 - Ref.: 7-1"
