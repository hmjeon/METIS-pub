...
module Mani
    ...
    public Mani_Set_2DGeo_Ori
    public Mani_Set_3DGeo_Ori
    public Space
...
subroutine Mani_Set_Prob(prob, color)
    ...
    if(prob.sel_edge_sec == 3) char_sec = "6HBm"

    prob.name_file = trim(prob.name_prob)//"_"//trim(char_sec)//"_"//&
        trim(adjustl(trim(char_bp)))//"bp"

    if(color == 'blue') then
        prob.color      = [ 114, 147, 203 ]     ! [  57, 106, 177 ]
        prob.color_atom = [  52, 152, 219 ]
    else if(color == 'red') then
        prob.color      = [ 211,  94,  96 ]     ! [ 204,  37,  41 ]
        prob.color_atom = [ 231,  76,  60 ]
    else if(color == 'green') then
        prob.color      = [ 132, 186,  91 ]     ! [  62, 150,  81 ]
        prob.color_atom = [  77, 175,  74 ]
    else if(color == 'yellow') then
        prob.color      = [ 204, 194,  16 ]     ! [ 148, 139,  61 ]
        prob.color_atom = [ 247, 147,  30 ]
    else if(color == 'violet') then
        prob.color      = [ 144, 103, 167 ]     ! [ 107,  76, 154 ]
        prob.color_atom = [ 150,  58, 228 ]
    end if
end subroutine Mani_Set_Prob

! -----------------------------------------------------------------------------

! Set the orientation of the 2D geometry
subroutine Mani_Set_2DGeo_Ori(geom, pseudo, angle)
...
end subroutine Mani_Set_2DGeo_Ori

! -----------------------------------------------------------------------------

! Set the orientation of the 3D geometry
subroutine Mani_Set_3DGeo_Ori(geom, pseudo, angle)
    type(GeomType), intent(inout) :: geom
    double precision, intent(in)  :: pseudo(3)
    double precision, intent(in)  :: angle
    ...
    do i = 1, geom.n_iniP
        call Rotate_Vector(geom.iniP(i).pos(:), pseudo, angle * pi / 180.0d0)
    end do
end subroutine Mani_Set_3DGeo_Ori
