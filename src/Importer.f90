...
subroutine Importer_GEO(prob, geom)
    ...
    if(prob.type_file == 'geo' .or. prob.type_file == 'GEO') then
    ...
    if(geom.n_face == 0) then
        if(prob.type_file == 'geo' .or. prob.type_file == 'GEO') close(unit = 1002)
        ...
        !DEC$ ELSE
        results = systemqq("python /home/polyhedra/"//trim(p_designer)//&
            "/build/tools/Shapely/Shapely.py "//trim(fullname))
        !results = systemqq("python tools/Shapely/Shapely.py "//trim(fullname))
        !DEC$ ENDIF
        if(results == .false.) then
            write(p_redir, "(a)")
    ...
    if(geom.n_face == 1) then
        ...
        if(p_mesh >= 0.0d0 .and. p_mesh < 1.0) then
            ...
            results = systemqq("/usr/local/matlab/R2017a/bin/matlab wait -nodisplay -nosplash -nodesktop -noawt -r "//&
                '"addpath /home/polyhedra/'//trim(p_designer)//'/build/tools/DistMesh/src; addpath /home/polyhedra/'//&
                trim(p_designer)//'/build/tools/DistMesh; DistMesh_linux('&
                //"'"//trim(fullname)//"',"//trim(Dble2Str(p_mesh))//')"')
...
subroutine Importer_SVG(prob, geom)
    ...
    ! Read number of points and faces
    n_line = 0
    if(prob.type_file == 'svg' .or. prob.type_file == 'SVG') then
