...
subroutine Basepair_Chimera_Cylinder(prob, geom, mesh, mode)
    ...
    integer :: i, j, node, node_1, node_2, min_croL, gap_layer = 0
        ...
        do j = 1, n_conn
            ...
            if(type_conn(j) == 2) then
                ...
                if(para_vertex_design == "mitered") then
                    ...
                    ! Increase the length of the basepair to fill the gap at the vertex
...
! Increase the length of the basepair to fill the gap at the vertex
subroutine Basepair_Increase_Edge_v02(prob, geom, mesh, node_cur, node_com)
    type(probType), intent(in)    :: prob
    type(geomType), intent(inout) :: geom
    type(MeshType), intent(inout) :: mesh
    integer,        intent(in)    :: node_cur
    ...
    if(dabs(a - b) < 0.34d0) then
        ...
        if(mod(nint(length / para_dist_bp) + 2, 21) /= 0 .and. mod(nint(length / para_dist_bp) + 2, 21) /= 10) then
            mesh.node(node_in).conn = 3
        end if
...
end subroutine Basepair_Increase_Edge_v02
